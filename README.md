...
PoC Assets panel では、現在の workspace / playbook / connector 設定から README、proposal section、follow-up email、Mermaid architecture diagram、`.env.example`、SQL、Python、Terraform、checklist、troubleshooting guide の starter template を生成できます。生成物は preview と clipboard export のみで、OCI API、DB 接続、Terraform apply は実行しません。

Knowledge には browser capture に加えて `.md` / `.txt` 文書を追加できます。Electron 実行時は file dialog から読み込み、renderer preview ではブラウザーの file input で読み込みます。現在の上限は 1 MB です。
