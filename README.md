...
Grounded Answer の回答本文は既定では deterministic ですが、OCI Generative AI (Enterprise AI project) settings が揃うと、検索済み context を根拠とした実生成回答へ差し替わります。OCI GenAI は OpenAI 互換 API として main process から直接呼び出します（oci-sdk は使用しません）。credential は settings store / Electron safeStorage に保持し、renderer の localStorage、UI、prompt には返しません。

1. Browser Client の Settings で Base URL、model、Project、API key を保存する。
2. `保存してテスト` で保存済み設定を使った最小 chat completion を実行し、endpoint / model / API key が実際に利用できるか確認する。
3. local-keyword / Oracle Vector いずれの adapter でも、検索 context が 1 件以上ある場合に `generateOciGenAiAnswer` が回答を実生成し、`answerProvider: "oci-genai"` を返します。設定不足・呼び出し失敗時は deterministic answer にそのまま fallback します。実行ロジックは `src/main/ociGenAiExecutor.ts` にあり、純粋関数（readiness 判定・prompt 生成・接続テスト）は単体テスト済みです。
